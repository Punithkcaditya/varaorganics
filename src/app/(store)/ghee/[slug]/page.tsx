...
export function generateStaticParams() {
  return productStaticParams("ghee");
}
...
export async function generateMetadata({
  ...
  return productMetadata("ghee", slug);
}

export default async function GheeProductPage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const resolved = await resolveProductRoute("ghee", slug);
